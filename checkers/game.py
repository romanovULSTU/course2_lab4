...
class Game:
    def __init__(self, canvas: Canvas, x_field_size: int, y_field_size: int):
        ...
        if (PLAYER_SIDE == SideType.BLUE):
            self.__handle_enemy_turn()

    def __init_images(self):
        '''Инициализация изображений'''
        self.__images = {
            CheckerType.RED_REGULAR: ImageTk.PhotoImage(Image.open(Path('assets', 'RR.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.BLUE_REGULAR: ImageTk.PhotoImage(Image.open(Path('assets', 'BR.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.RED_QUEEN: ImageTk.PhotoImage(Image.open(Path('assets', 'RQ.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
            CheckerType.BLUE_QUEEN: ImageTk.PhotoImage(Image.open(Path('assets', 'BQ.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS)),
        }
    ...
    def mouse_down(self, event: Event):
        ...
        if (PLAYER_SIDE == SideType.RED):
            player_checkers = RED_CHECKERS
        elif (PLAYER_SIDE == SideType.BLUE):
    ...
    def __predict_optimal_moves(self, side: SideType) -> list[Move]:
            ...
            for moves in predicted_moves_list:
                ...
                try:
                    if (side == SideType.RED):
                        result = self.__field.red_score / self.__field.blue_score
                    elif (side == SideType.BLUE):
                        result = self.__field.blue_score / self.__field.red_score
        ...
        if (optimal_moves):
            ...
            for move in choice(optimal_moves):
                if   (side == SideType.RED and self.__field.type_at(move.from_x, move.from_y) in BLUE_CHECKERS): break
                elif (side == SideType.BLUE and self.__field.type_at(move.from_x, move.from_y) in RED_CHECKERS): break
